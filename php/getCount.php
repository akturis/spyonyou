...
$count = mysql_query('SELECT DATE_FORMAT(logon,\'%e %b %Y\') dat, COUNT(*) count FROM (SELECT DISTINCT date(logonDateTime) logon, characterId FROM `logons` WHERE logonDateTime>(CURDATE() - INTERVAL 12 MONTH)) dist GROUP BY logon ORDER BY COUNT(*) DESC LIMIT 1');
