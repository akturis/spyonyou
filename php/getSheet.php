...
if ($char = mysql_fetch_assoc($char)) {
  ...
  $mis = mysql_query('SELECT COUNT(*) missions FROM `entries` WHERE date BETWEEN NOW() - INTERVAL '.$days.' DAY AND NOW() AND characterID='.$id);
  ...
  $char = mysql_query('SELECT logonDateTime,(CASE WHEN TIMEDIFF(logoffDateTime,logonDateTime)>0 THEN TIMEDIFF(logoffDateTime,logonDateTime) ELSE 0 END) time,location,shipType FROM `logons` WHERE characterId='.$id.' AND logonDateTime BETWEEN NOW() - INTERVAL '.$days.' DAY AND NOW() ORDER BY logonDateTime DESC');
